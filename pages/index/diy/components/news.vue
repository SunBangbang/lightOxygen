		...
		<view class='news acea-row row-middle' :class="{pageOn:bgStyle===1}" v-if="itemNew.length" :style="'margin:0 '+prConfig*2+'rpx;margin-top:'+mbConfig*2+'rpx;color:'+txtColor+';background-color:'+bgColor+';'">
			...
			<view class='swiperTxt skeleton-rect'>
				<block v-for="(item,index) in itemNew" :key='index'>
					<u-notice-bar mode="horizontal" :list="list" :volume-icon="false" type="none"></u-notice-bar>
				</block>
...
<script>
	export default {
		...
		created() {
			this.itemNew.forEach((item,index)=>{    //js遍历数组
				console.log(item.chiild[0].val,'item11')
				this.list[0] = item.chiild[0].val
			});
		},
		mounted() {},
		methods: {
...
<style lang="scss">
	/deep/.u-notice-bar[data-v-4661e472]{
		padding:0!important;
	}
	/deep/.u-notice-text{
		color:red!important;
